...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for flight_info in soup.find_all("div", class_="tr"):
            flight_number = flight_info.find(
                "div", class_="td col-xs-3 col-sm-2 col-md-2"
            ).text.strip()
            destination = (
                flight_info.find("div", class_="td col-xs-4 col-sm-4 col-md-4")
                .text.strip()
                .split("(")[0]
                .strip()
            )
            scheduled_departure = flight_info.find(
                "div", class_="sch"
            ).text.strip()
            ...
            scheduled_datetime_str = (
                f"{scheduled_time_str}, {scheduled_date_str}"
            )
            departure_time = datetime.strptime(
                scheduled_datetime_str, "%H:%M, %d %b"
            )
