...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for flight_info in soup.select(
            "selector-for-flight-info"
        ):  # Adjust this selector
            flight_number = flight_info.select_one(
                "selector-for-flight-number"
            ).text
            destination = flight_info.select_one(
                "selector-for-destination"
            ).text
            departure_time = datetime.strptime(
                flight_info.select_one("selector-for-departure-time").text,
                "%Y-%m-%d %H:%M:%S",
            )

            arrival_time = departure_time + timedelta(
                hours=2
            )  # Assuming average flight duration
            flights.append(
                Flight(
        ...
        self.stdout.write(
            self.style.SUCCESS("Successfully fetched flight information")
        )
